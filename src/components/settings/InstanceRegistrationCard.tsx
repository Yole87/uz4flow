...
export function InstanceRegistrationCard() {
  ...
  const createInstance = useMutation({
    mutationFn: async () => {
      ...
      // 1. Create instance without the API key
      ...
      // 2. Encrypt API key server-side via edge function
    ...
    <>
      ...
      <Card className="border-dashed border-2 border-primary/30 bg-primary/5 hover:bg-primary/10 transition-colors cursor-pointer"
        ...
        <CardContent className="flex flex-col items-center justify-center py-8 gap-3">
          ...
          <span className="text-sm font-semibold text-foreground">Cadastrar Credenciais da Instância</span>
          <span className="text-xs text-muted-foreground text-center max-w-xs">
            Adicione uma nova instância WhatsApp AI para gerenciar mensagens
          </span>
        </CardContent>
      </Card>
