...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { organization_id, start, end } = body;

    if (!organization_id || !start || !end) {
    ...
    if (!response.ok) {
      console.error("[GCal-List] API error:", data);
      return new Response(JSON.stringify({ error: "Failed to list calendar events", details: data }), {
        status: 500,
        headers: { ..._corsHeaders, "Content-Type": "application/json" },
      });
    }
